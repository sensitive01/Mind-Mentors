...
const RelFacultyProgram = require("../../model/kimsdatabase/relBtwFacultyProgram");

const importRelFacultyProgramExcelWithReport = async () => {
  try {
    ...
    const filePath = path.join(
      __dirname,
      "../../../../kimsdata/opt#Programs#Faculty.csv" // <-- update your Excel path
    );
    ...
    const formattedData = jsonData.map((row, index) => ({
      __row: index + 2,
      FacultyID: row.RECORD_LINK_ID_Base ? String(row.RECORD_LINK_ID_Base) : null,
    ...
    try {
      const insertedDocs = await RelFacultyProgram.insertMany(formattedData, {
        ordered: false,
      });
      ...
      if (err.writeErrors && err.writeErrors.length > 0) {
        ...
        errorDetails = err.writeErrors.map((we) => ({
          ...
          FacultyID: formattedData[we.index].FacultyID,
...
importRelFacultyProgramExcelWithReport();
