...
const EmployeeHolidayForm = () => {
    const [holidayDetails, setHolidayDetails] = useState([
        { id: 1, holidayName: '', startDate: '', endDate: '', description: '', status: '' }
    ]);
    ...
    const addHolidayDetail = () => {
        setHolidayDetails([
            ...holidayDetails,
            { id: holidayDetails.length + 1, holidayName: '', startDate: '', endDate: '', description: '', status: '' }
        ]);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            for (let holiday of holidayDetails) {
                ...
                if (holiday.holidayName && holiday.startDate && holiday.endDate && holiday.description && holiday.status) {
                    const holidayData = {
            ...
            setHolidayDetails([{ id: 1, holidayName: '', startDate: '', endDate: '', description: '', status: '' }]);
                        ...
                        {holidayDetails.map((holiday, index) => (
                            <div key={holiday.id} className="space-y-4">
                                <div className="flex items-center justify-between">
                                    <label className="text-sm font-medium text-[#642b8f]">Holiday {index + 1}</label>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <TextField
                                        ...
                                        onChange={(e) => handleHolidayChange(holiday.id, 'startDate', e.target.value)}
                                        fullWidth
                                    ...
                                    <TextField
                                        ...
                                        onChange={(e) => handleHolidayChange(holiday.id, 'endDate', e.target.value)}
                                        fullWidth
                                        InputLabelProps={{ shrink: true }}
                                    />
                                    <TextField
                                        ...
                                        onChange={(e) => handleHolidayChange(holiday.id, 'status', e.target.value)}
                                        fullWidth
                                    >
