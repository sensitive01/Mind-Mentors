...
import { attandaceData, markAttendance } from "../../../api/service/employee/EmployeeService";
...
const AttendanceCalendar = () => {
  ...
  useEffect(() => {
    const fetcAttandaceData = async () => {
      const response = await attandaceData(email);
  ...
  const handleMarkAttendance = async () => {
    ...
    try {
      const formData = { employeeName, email };
  ...
  const generateCalendarDays = (year, month) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (attendanceDate < currentDate) {
        status = storedAttendance === "true" ? "present" : "absent";
      } else {
        status = "yet to mark";
      }
